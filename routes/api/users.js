...
router.post('/user-add', (req, res) => {
    ...
    User.findOne({ email: req.body.email }).then(user => {
        if (user) {
            return res.status(400).json({ email: 'Number already exists' });
        } else {
            ...
            //     bcrypt.hash(newUs, salt, (err, hash) => {
            //         if (err) throw err;
...
router.post('/user-delete', (req, res) => {
    User.findByIdAndDelete({ _id: req.body._id }).then(user => {
        if (user) {
...
router.post('/user-update', (req, res) => {
    ...
    User.findOne({ _id }).then(user => {
        if (user) {
            ...
            //     });
            // }
            User.findOne({ email: req.body.email }).then(numberFound => {
                if (numberFound) {
                    return res.status(400).json({ email: 'Number already exists.' });
                } else {
                    let update = { 'name': req.body.name, 'email': req.body.email };
                    User.findByIdAndUpdate({ _id: _id }, { $set: update }, function (err, result) {
                        if (err) {
